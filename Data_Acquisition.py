...
Ch03_name = 'Sync'
num_channels = 2
fs_acq = 40000 #sample frequency of 40kHz
bufsize = 40000
...
with nidaqmx.Task() as task:
    ...
    def reading_task_callback(task_idx, event_type, num_samples, callback_data=None):
        ...
        data = np.append(data,buffer.astype(np.float64),axis=1)
    ...
    try:
        ...
        while True:
            choice = input('Ready for Test {}? (y/n) '.format(test_count))
            ...
            if choice == 'n' or choice == 'N':
                ...
                df = pd.DataFrame(data.T,columns=['Current Sensor','Load Cell'])
                df.to_csv('FinalData_Collection.csv')
                for i in range(20):
